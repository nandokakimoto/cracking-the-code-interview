...
describe('Sum Lists - Recursive', function() {
  ...
  describe('same length', function(){
    ...
    it('should sum values greater than 10', function(){
      ...
      assert.equal(sum.data, 1);
      assert.equal(sum.next.data, 8);
      assert.equal(sum.next.next.data, 8);
      assert.equal(sum.next.next.next.data, 8);
  ...
  describe('different length', function(){
    it('should sum values less than 10', function(){
      ...
      var node2 = new Node(9).append(9).append(5);

      var sum = sumLists(node1, node2);

      assert.equal(sum.data, 9);
    ...
    it('should sum values greater than 10', function(){
      var node1 = new Node(9).append(9).append(5);
      var node2 = new Node(2).append(9).append(8).append(5);

      var sum = sumLists(node1, node2);

      assert.equal(sum.data, 3);
      assert.equal(sum.next.data, 9);
      assert.equal(sum.next.next.data, 8);
      assert.equal(sum.next.next.next.data, 0);
      assert.equal(sum.next.next.next.next, null);
    });
  });
});
